...
generarPolvoCristal();
    ...
    // ── RAMO INTERACTIVO ──
    // Sacamos el tooltip al <body> para escapar del contexto 3D de cara-dorso
    // (transform-style: preserve-3d rompe position:absolute y z-index)
    ...
    if (tooltipRamo) {
        ...
        tooltipRamo.style.right    = "";
    }

    function mostrarTooltipRamo() {
        if (!tooltipRamo || !ramoInteractivo) return;
        ...
        tooltipRamo.style.transform = "translate(-50%, -100%) scale(1)";
    ...
    if (ramoInteractivo) {
        ...
        ramoInteractivo.addEventListener("touchstart", (e) => {
            e.stopPropagation();
            e.preventDefault();
            mostrarTooltipRamo();
            ramoInteractivo.classList.remove("brillo");
