...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ── FIRMA INTERACTIVA (mensajes al tocar "Edgar") ──
    const ramoInteractivo = document.getElementById("ramo-interactivo");
    const firmaTyping     = document.getElementById("firma-typing");
    const tooltipRamo     = document.getElementById("tooltip-ramo");
    let tooltipTimer;

    // Sacamos el tooltip al <body> para que no quede cortado por overflow:hidden ni por el contexto 3D
    if (tooltipRamo && tooltipRamo.parentNode !== document.body) {
        document.body.appendChild(tooltipRamo);
    }
    if (tooltipRamo) {
        tooltipRamo.style.position = "fixed";
        tooltipRamo.style.zIndex   = "99999";
    }

    function mostrarMensajeFirma(e) {
        e.stopPropagation();
        if (e.type === 'touchstart') e.preventDefault();

        if (!tooltipRamo || !firmaTyping) return;

        // Posición real de la firma en pantalla
        const rect = firmaTyping.getBoundingClientRect();
        tooltipRamo.style.left      = `${rect.left + rect.width / 2}px`;
        tooltipRamo.style.top       = `${rect.top - 12}px`;
        tooltipRamo.style.transform = "translate(-50%, -100%)";
        tooltipRamo.style.bottom    = "";
        ...
        tooltipTimer = setTimeout(() => {
            tooltipRamo.classList.remove("visible");
        }, 2600);
    }

    if (firmaTyping) {
        firmaTyping.style.cursor = "pointer";
        firmaTyping.addEventListener("click",      mostrarMensajeFirma);
        firmaTyping.addEventListener("touchstart", mostrarMensajeFirma, { passive: false });
    }
