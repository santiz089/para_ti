...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function activarRamo(e) {
        ...
        if (tooltipRamo && ramoInteractivo) {
            // Calculamos la posición real del ramo en pantalla
            const rect = ramoInteractivo.getBoundingClientRect();
            const tooltipLeft = rect.left + rect.width / 2;
            const tooltipTop  = rect.top - 14; // un poco encima del ramo

            tooltipRamo.style.left      = `${tooltipLeft}px`;
            tooltipRamo.style.top       = `${tooltipTop}px`;
